...
比较检验，更准确地称为假设检验的比较，通常是指一组统计方法，用于确定两个或多个数据集或样本群是否有显著差异。比较检验可以用于许多不同类型的数据和情境，目的是判断观察到的差异是否仅仅是随机的结果，还是真实存在统计上的显著差异。

以下是一些常用的比较检验：

1. **t检验（t-test）**:
   - **独立样本t检验（Independent samples t-test）**：用于比较两个独立样本的均值是否存在显著差异。
   - **配对样本t检验（Paired samples t-test）**：用于比较同一组个体在两种不同条件下的均值是否存在显著差异。
2. **方差分析（ANOVA）**:
   - 用于比较三个或更多群体的均值是否存在显著差异。
3. **卡方检验（Chi-square test）**:
   - 用于比较两个分类变量之间的独立性或关联性。
4. **曼-惠特尼U检验（Mann-Whitney U test）**:
   - 非参数检验，用于比较两个独立样本的中位数。常用于不满足正态分布假设的数据。
5. **威尔科克森符号秩检验（Wilcoxon signed-rank test）**:
   - 非参数检验，用于比较两个配对样本的中位数。
6. **克鲁斯卡尔-瓦里斯检验（Kruskal-Wallis test）**:
   - 非参数的ANOVA，用于比较三个或更多样本群的中位数。

比较检验通常是研究中用于测试特定假设或问题的关键部分。选择合适的检验取决于数据的类型、分布以及研究的设计。
...
偏差-方差分解是机器学习中一个重要的概念，用于解释模型的总误差是如何由三个主要部分组成的：偏差（Bias）、方差（Variance）和不可减少的误差。这三者之和就构成了模型的总误差。

1. **偏差（Bias）**：
   - 偏差衡量了模型的预测值与真实值之间的平均差异，即模型的预测值是否系统地偏离真实值。
   - 高偏差通常意味着模型太简单（欠拟合），不能捕获数据中的复杂模式。
2. **方差（Variance）**：
   - 方差衡量了模型对不同训练集的敏感性，即使用不同的训练数据，模型的预测是否会有很大的变化。
   - 高方差通常意味着模型过于复杂（过拟合），会对训练数据中的随机噪声敏感。
3. **不可减少的误差（Irreducible Error）**：
   - 这部分误差是由于数据本身的噪声所导致的，与模型无关。
   - 即使有完美的模型，这部分误差也无法避免。
...
$$
在实际应用中，存在一个偏差与方差之间的权衡关系：

- 如果模型太简单，可能会有很高的偏差，但方差可能会很低。
- 如果模型太复杂，可能会有很高的方差，但偏差可能会很低。

理解偏差和方差对于机器学习模型的选择和调优是非常重要的。通常，我们希望找到一个适中的模型，既不过于简单也不过于复杂，从而在偏差和方差之间实现平衡，以最小化总误差。
